  ...
  return (
    <div className="App" style={{ backgroundImage: `url(${currentImage})` }}>
      <div className="overlays">
        {revealed.map((isRevealed, index) => (
          !isRevealed && (
            ...
            >
              <div className="question">{questions.day6[index].question}</div>
              <input
                type={questions.day6[index].type === 'number' ? 'number' : 'text'}
      ...
      <div className="navigation-buttons">
        <button onClick={() => navigate('/day-5')} className="nav-button prev-button">← Previous</button>
        <button onClick={() => navigate('/day-7')} className="nav-button next-button">Next →</button>
      </div>
