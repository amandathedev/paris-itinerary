...
const Day6 = () => {
  const [revealed, setRevealed] = useState(
    JSON.parse(localStorage.getItem('day6_revealed')) || [false, false, false, false, false]
  );
  ...
  const revealItem = useCallback((index) => {
    ...
    localStorage.setItem('day6_revealed', JSON.stringify(newRevealed));
  }, [revealed]);

  const handleSubmit = (index) => {
    if (checkAnswer('day6', index, userAnswers[index])) {
      revealItem(index);
    } else {
  ...
  const currentImage = `${process.env.PUBLIC_URL}/images/day-6.png`;

  useEffect(() => {
    if ("geolocation" in navigator) {
      navigator.geolocation.getCurrentPosition(
        (position) => {
          questions.day6.forEach((question, index) => {
        ...
        () => {
          setGeolocationFailed(geolocationFailed.map((item, i) => questions.day6[i].type === "geolocation" ? true : item));
        }
      );
    }
  ...
  return (
    <div className="App" style={{ backgroundImage: `url(${currentImage})` }}>
      <div className="overlays">
        {questions.day6.map((question, index) => (
          !revealed[index] && (
            <div
              key={index}
      ...
      <div className="navigation-buttons">
        <button onClick={() => navigate('/day-5')} className="nav-button prev-button">←</button>
        <button onClick={() => navigate('/day-7')} className="nav-button next-button">→</button>
      </div>
